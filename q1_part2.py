...
for epoch in range(n_epochs):
    ...
    alpha_square = np.sum(np.power(W1,2))
    beta_square = np.sum(np.power(W2,2))

    loss = compute_loss(Y_vec, activation_l2) + lamda*(alpha_square + beta_square)
    #print loss
    ...
    W1 = W1 - lrate*(d_loss_d_alpha + 2*lamda*W1)
    W2 = W2 - lrate*(d_loss_d_beta + 2*lamda*W2)
